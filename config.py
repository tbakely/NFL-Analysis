...
# Archive views
CREATE_VIEWS_ARCHIVE = """
/* snap counts */
create view archive_data.offense_snap_counts as
select 
ids.id,
season,
week,
offense_snaps,
offense_pct
from archive_data.snap_count_data
left join
	(select distinct name, gsis_id as id from archive_data.full_ids) ids
on archive_data.snap_count_data.player = ids.name;


/* weekly_qb */
create view archive_data.weekly_qb as
select
	player_name,
	position,
	recent_team,
...
left join archive_data.ngs_passing_data ngs
on wd.player_id = ngs.player_gsis_id
and wd.season = ngs.season
and wd.week = ngs.week
...
and wd.season = os.season
and wd.week = os.week
left join (select distinct passer_player_id, game_id, season, week from archive_data.full_pbp) game_id
on wd.player_id = game_id.passer_player_id
and wd.season = game_id.season
and wd.week = game_id.week
...
/* weekly wr stats */
create view archive_data.weekly_wr as
select
	player_id,
	player_name,
	position,
...
and wd.season = os.season
and wd.week = os.week
left join archive_data.ngs_receiving_data ngs
on wd.player_id = ngs.player_gsis_id
and wd.season = ngs.season
and wd.week = ngs.week
left join (select distinct receiver_player_id, game_id, season, week from archive_data.full_pbp) game_id
on wd.player_id = game_id.receiver_player_id
and wd.season = game_id.season
and wd.week = game_id.week
left join archive_data.game_data
...
/* weekly te stats */
create view archive_data.weekly_te as
select
	player_id,
	player_name,
	position,
...
/* weekly rb stats */
create view archive_data.weekly_rb as
select
	player_id,
	player_name,
	position,
...
/* redzone snaps current season */
create view archive_data.redzone_snaps as
select
player_id,
player_name,
season,
